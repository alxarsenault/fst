...
FST_BEGIN_SUB_NAMESPACE(os)
    ...
    __fst::status copy_file(const __fst::os::path& old_name, const __fst::os::path& new_name, bool fail_if_exists) noexcept
    {
        ...
        if (CopyFileW(__fst::utf_cvt_as<os::wstring_type>(old_name._path).c_str(), __fst::utf_cvt_as<os::wstring_type>(new_name._path).c_str(), fail_if_exists))
        {
    ...
    {
        // If the function succeeds, the return value is nonzero.
        if (CreateDirectoryW(__fst::utf_cvt_as<os::wstring_type>(name._path).c_str(), nullptr)) { return __fst::status_code::success; }
    ...
    {
        // If the function succeeds, the return value is nonzero.
        if (DeleteFileW(__fst::utf_cvt_as<os::wstring_type>(name._path).c_str())) { return __fst::status_code::success; }
