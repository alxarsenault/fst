...
l_sinline int auxgetstr(lua_State* L, const TValue* t, FST_NAMESPACE::string_view k)
{
  const TValue* slot;
  TString* str = luaS_new(L, k);
  if (luaV_fastget(L, t, str, slot, luaH_getstr))
...
LUA_API void lua_createtable (lua_State *L, int narray, int nrec) {
  ...
  api_incr_top(L);
  if (narray > 0 || nrec > 0) luaH_resize(L, t, (unsigned int) narray, (unsigned int) nrec);
  luaC_checkGC(L);
  lua_unlock(L);
}
