...
namespace
{
    ...
    {
        //! Current amount of virtual memory mapped, all of which might not have
        //! been committed (only if ENABLE_STATISTICS=1)
        size_t mapped;
    ...
    {
        //! Map memory pages for the given number of bytes. The returned address
        //! MUST be
        //  aligned to the rpmalloc span size, which will always be a power of two.
        //  Optionally the function can store an alignment offset in the offset
        //  variable in case it performs alignment and the returned pointer is
        //  offset from the actual start of the memory region due to this alignment.
        //  The alignment offset will be passed to the memory unmap function. The
        //  alignment offset MUST NOT be larger than 65535 (storable in an
        //  uint16_t), if it is you must use natural alignment to shift it into 16
        //  bits. If you set a memory_map function, you must also set a memory_unmap
        //  function or else the default implementation will be used for both. This
        //  function must be thread safe, it can be called by multiple threads
        //  simultaneously.
        ...
        //  returned by a previous call to memory_map and that the entire range
        //  should be released. The release argument holds the size of the entire
        //  span range. If release is set to 0, the unmap is a partial decommit of a
        //  subset of the mapped memory range. If you set a memory_unmap function,
        //  you must also set a memory_map function or else the default
        //  implementation will be used for both. This function must be thread safe,
        //  it can be called by multiple threads simultaneously.
        void (*memory_unmap)(void* address, size_t size, size_t offset, size_t release);
        //! Called when an assert fails, if asserts are enabled. Will use the
        //! standard assert()
        //  if this is not set.
        ...
        //  retried. The argument passed is the number of bytes that was requested
        //  in the map call. Only used if the default system memory map function is
        //  used (memory_map callback is not set).
        ...
        //  page size. Used if RPMALLOC_CONFIGURABLE is defined to 1, otherwise
        //  system page size is used.
        size_t page_size;
        ...
        //! This can
        //  be used to minimize the system call overhead at the cost of virtual
        //  memory address space. The extra mapped pages will not be written until
        //  actually used, so physical committed memory should not be affected in
        //  the default implementation. Will be aligned to a multiple of spans that
        //  match memory page size in case of huge pages.
        ...
        //  configuration. If this is set to non-zero and page_size is also
        //  non-zero, the allocator will assume huge pages have been configured and
        //  enabled prior to initializing the allocator. For Windows, see
    ...
    RPMALLOC_EXPORT static int rpmalloc_initialize(void);

    //! Initialize allocator with given configuration
    RPMALLOC_EXPORT static int rpmalloc_initialize_config(const rpmalloc_config_t* config);

    //! Get allocator configuration
    RPMALLOC_EXPORT static const rpmalloc_config_t* rpmalloc_config(void);

    //! Finalize allocator
    RPMALLOC_EXPORT static void rpmalloc_finalize(void);

    //! Initialize allocator for calling thread
    RPMALLOC_EXPORT static void rpmalloc_thread_initialize(void);

    //! Finalize allocator for calling thread
    RPMALLOC_EXPORT static void rpmalloc_thread_finalize(int release_caches);

    //! Perform deferred deallocations pending for the calling thread heap
    RPMALLOC_EXPORT static void rpmalloc_thread_collect(void);

    //! Query if allocator is initialized for calling thread
    RPMALLOC_EXPORT static int rpmalloc_is_thread_initialized(void);

    //! Get per-thread statistics
    RPMALLOC_EXPORT static void rpmalloc_thread_statistics(rpmalloc_thread_statistics_t* stats);

    //! Get global statistics
    RPMALLOC_EXPORT static void rpmalloc_global_statistics(rpmalloc_global_statistics_t* stats);

    //! Dump all statistics in human readable format to file (should be a FILE*)
    RPMALLOC_EXPORT static void rpmalloc_dump_statistics(void* file);

    //! Allocate a memory block of at least the given size
    RPMALLOC_EXPORT RPMALLOC_ALLOCATOR static void* rpmalloc(size_t size) RPMALLOC_ATTRIB_MALLOC RPMALLOC_ATTRIB_ALLOC_SIZE(1);

    //! Free the given memory block
    RPMALLOC_EXPORT static void rpfree(void* ptr);

    //! Allocate a memory block of at least the given size and zero initialize it
    RPMALLOC_EXPORT RPMALLOC_ALLOCATOR static void* rpcalloc(size_t num, size_t size) RPMALLOC_ATTRIB_MALLOC RPMALLOC_ATTRIB_ALLOC_SIZE2(1, 2);

    //! Reallocate the given block to at least the given size
    RPMALLOC_EXPORT RPMALLOC_ALLOCATOR static void* rprealloc(void* ptr, size_t size) RPMALLOC_ATTRIB_MALLOC RPMALLOC_ATTRIB_ALLOC_SIZE(2);
    ...
    //  (default 64KiB)
    RPMALLOC_EXPORT RPMALLOC_ALLOCATOR static void* rpaligned_realloc(void* ptr, size_t alignment, size_t size, size_t oldsize, unsigned int flags) RPMALLOC_ATTRIB_MALLOC
    ...
    //  internals is that this must also be strictly less than the span size
    //  (default 64KiB)
    RPMALLOC_EXPORT RPMALLOC_ALLOCATOR static void* rpaligned_alloc(size_t alignment, size_t size) RPMALLOC_ATTRIB_MALLOC RPMALLOC_ATTRIB_ALLOC_SIZE(2);
    ...
    //  internals is that this must also be strictly less than the span size
    //  (default 64KiB)
    RPMALLOC_EXPORT RPMALLOC_ALLOCATOR static void* rpaligned_calloc(size_t alignment, size_t num, size_t size) RPMALLOC_ATTRIB_MALLOC RPMALLOC_ATTRIB_ALLOC_SIZE2(2, 3);
    ...
    //  internals is that this must also be strictly less than the span size
    //  (default 64KiB)
    RPMALLOC_EXPORT RPMALLOC_ALLOCATOR static void* rpmemalign(size_t alignment, size_t size) RPMALLOC_ATTRIB_MALLOC RPMALLOC_ATTRIB_ALLOC_SIZE(2);
    ...
    //  internals is that this must also be strictly less than the span size
    //  (default 64KiB)
    RPMALLOC_EXPORT static int rpposix_memalign(void** memptr, size_t alignment, size_t size);

    //! Query the usable size of the given memory block (from given pointer to the
    //! end of block)
    RPMALLOC_EXPORT static size_t rpmalloc_usable_size(void* ptr);

    //! Dummy empty function for forcing linker symbol inclusion
    RPMALLOC_EXPORT static void rpmalloc_linker_reference(void);
    ...
    RPMALLOC_EXPORT static rpmalloc_heap_t* rpmalloc_heap_acquire(void);
    ...
    //  a null pointer.
    RPMALLOC_EXPORT static void rpmalloc_heap_release(rpmalloc_heap_t* heap);

    //! Allocate a memory block of at least the given size using the given heap.
    RPMALLOC_EXPORT RPMALLOC_ALLOCATOR static void* rpmalloc_heap_alloc(rpmalloc_heap_t* heap, size_t size) RPMALLOC_ATTRIB_MALLOC RPMALLOC_ATTRIB_ALLOC_SIZE(2);
    ...
    RPMALLOC_EXPORT RPMALLOC_ALLOCATOR static void* rpmalloc_heap_aligned_alloc(rpmalloc_heap_t* heap, size_t alignment, size_t size) RPMALLOC_ATTRIB_MALLOC
        RPMALLOC_ATTRIB_ALLOC_SIZE(3);

    //! Allocate a memory block of at least the given size using the given heap and
    //! zero initialize it.
    RPMALLOC_EXPORT RPMALLOC_ALLOCATOR static void* rpmalloc_heap_calloc(rpmalloc_heap_t* heap, size_t num, size_t size) RPMALLOC_ATTRIB_MALLOC
    ...
    RPMALLOC_EXPORT RPMALLOC_ALLOCATOR static void* rpmalloc_heap_aligned_calloc(rpmalloc_heap_t* heap, size_t alignment, size_t num, size_t size) RPMALLOC_ATTRIB_MALLOC
        RPMALLOC_ATTRIB_ALLOC_SIZE2(2, 3);

    //! Reallocate the given block to at least the given size. The memory block MUST
    //! be allocated
    //  by the same heap given to this function.
    RPMALLOC_EXPORT RPMALLOC_ALLOCATOR static void* rpmalloc_heap_realloc(rpmalloc_heap_t* heap, void* ptr, size_t size, unsigned int flags) RPMALLOC_ATTRIB_MALLOC
        RPMALLOC_ATTRIB_ALLOC_SIZE(3);
    ...
    RPMALLOC_EXPORT RPMALLOC_ALLOCATOR static void* rpmalloc_heap_aligned_realloc(
    ...
    //! allocated
    //  by the same heap given to this function.
    RPMALLOC_EXPORT static void rpmalloc_heap_free(rpmalloc_heap_t* heap, void* ptr);

    //! Free all memory allocated by the heap
    RPMALLOC_EXPORT static void rpmalloc_heap_free_all(rpmalloc_heap_t* heap);
    ...
    //  other threads.
    RPMALLOC_EXPORT static void rpmalloc_heap_thread_set_current(rpmalloc_heap_t* heap);
...
#define rpmalloc_assume(cond)                                        \
    do                                                               \
    {                                                                \
        if (!__builtin_expect(cond, false)) __builtin_unreachable(); \
    } while (0)
#elif defined(_MSC_VER)
...
#define rpmalloc_assert(truth, message) fst_assert(truth, message)
...
//////
// FST_BEGIN_NAMESPACE
namespace //FST_RP_MALLOC_NAMESPACE
{
...
#define _rpmalloc_stat_inc_alloc(heap, class_idx)                                                                                   \
    ...
    {                                                                                                                               \
        ...
        if (alloc_current > heap->size_class_use[class_idx].alloc_peak) heap->size_class_use[class_idx].alloc_peak = alloc_current; \
...
#if ENABLE_PRELOAD
        if (EXPECTED(heap != 0)) return heap;
        rpmalloc_initialize();
...
#error This platform needs implementation of get_thread_id()
    ...
    static void NTAPI _rpmalloc_thread_destructor(void* value)
    {
        ...
        if (get_thread_id() == _rpmalloc_main_thread_id) return;
#endif
        if (value) rpmalloc_thread_finalize(1);
    }
#endif
    ...
    static void* _rpmalloc_mmap_os(size_t size, size_t* offset)
    {
        ...
        if (!ptr)
        {
            ...
            {
                if (_memory_config.map_fail_callback(size + padding)) return _rpmalloc_mmap_os(size, offset);
            }
            else { rpmalloc_assert(ptr, "Failed to map virtual memory block"); }
            return 0;
        }
#else
            ...
            if (_memory_huge_pages) fd |= VM_FLAGS_SUPERPAGE_SIZE_2MB;
            ...
            if ((ptr == MAP_FAILED) || !ptr)
            {
                ...
                else if (errno != ENOMEM) { rpmalloc_assert((ptr != MAP_FAILED) && ptr, "Failed to map virtual memory block"); }
                return 0;
            }
#endif
...
#if !DISABLE_UNMAP
#if PLATFORM_WINDOWS
        if (!VirtualFree(address, release ? 0 : size, release ? MEM_RELEASE : MEM_DECOMMIT)) { rpmalloc_assert(0, "Failed to unmap virtual memory block"); }
#else
        if (release)
        {
            if (munmap(address, release)) { rpmalloc_assert(0, "Failed to unmap virtual memory block"); }
        }
        else
        {
...
#endif
#endif
        if (release) _rpmalloc_stat_sub(&_mapped_pages_os, release >> _memory_page_size_shift);
    }
    ...
    static void _rpmalloc_span_double_link_list_remove(span_t** head, span_t* span)
    {
        ...
        if (*head == span) { *head = span->next; }
        else
        {
            span_t* next_span = span->next;
            span_t* prev_span = span->prev;
            prev_span->next = next_span;
            if (EXPECTED(next_span != 0)) next_span->prev = prev_span;
        }
    }
    ...
    static span_t* _rpmalloc_span_map_from_reserve(heap_t* heap, size_t span_count)
    {
        ...
        if (span_count <= LARGE_CLASS_COUNT) _rpmalloc_stat_inc(&heap->span_use[span_count - 1].spans_from_reserved);
    ...
    static span_t* _rpmalloc_span_map_aligned_count(heap_t* heap, size_t span_count)
    {
        ...
        // heap cache and map a new full set of spans. Otherwise we would waste
        // memory if page size > span size (huge pages)
        ...
        if (span_count <= LARGE_CLASS_COUNT) _rpmalloc_stat_inc(&heap->span_use[span_count - 1].spans_map_calls);
        if (aligned_span_count > span_count)
        {
            ...
            {
                // If huge pages or eager spam map count, the global reserve spin
                // lock is held by caller, _rpmalloc_span_map
    ...
    {
        if (span_count <= heap->spans_reserved) return _rpmalloc_span_map_from_reserve(heap, span_count);
        span_t* span = 0;
        ...
        {
            // If huge pages, make sure only one thread maps more memory to avoid
            // bloat
            ...
            if (_memory_global_reserve_count >= span_count)
            {
                ...
                if (_memory_global_reserve_count < reserve_count) reserve_count = _memory_global_reserve_count;
                ...
                if (span)
                {
                    ...
                    // Already marked as subspan in
                    // _rpmalloc_global_get_reserved_spans
                    span->span_count = (uint32_t) span_count;
                }
            }
        }
        if (!span) span = _rpmalloc_span_map_aligned_count(heap, span_count);
        if (use_global_reserve) atomic_store32_release(&_memory_global_lock, 0);
        return span;
    }
    ...
    static void _rpmalloc_span_unmap(span_t* span)
    {
        ...
        if (!is_master)
        {
            // Directly unmap subspans (unless huge pages, in which case we defer
            // and unmap entire page range with master)
            rpmalloc_assert(span->align_offset == 0, "Span align offset corrupted");
            if (_memory_span_size >= _memory_page_size) _rpmalloc_unmap(span, span_count * _memory_span_size, 0, 0);
        }
        else
        {
        ...
        {
            // Everything unmapped, unmap the master span with release flag to unmap
            // the entire range of the super span
            rpmalloc_assert(!!(master->flags & SPAN_FLAG_MASTER) && !!(master->flags & SPAN_FLAG_SUBSPAN), "Span flag corrupted");
            size_t unmap_count = master->span_count;
            if (_memory_span_size < _memory_page_size) unmap_count = master->total_spans;
    ...
    static void _rpmalloc_span_release_to_cache(heap_t* heap, span_t* span)
    {
        ...
        if (!heap->finalize)
        {
            ...
            if (heap->size_class[span->size_class].cache) _rpmalloc_heap_cache_insert(heap, heap->size_class[span->size_class].cache);
            heap->size_class[span->size_class].cache = span;
        }
        else { _rpmalloc_span_unmap(span); }
    }
    ...
    static void* _rpmalloc_span_initialize_new(heap_t* heap, heap_size_class_t* heap_size_class, span_t* span, uint32_t class_idx)
    {
        ...
        span->free_list_limit
            = free_list_partial_init(&heap_size_class->free_list, &block, span, pointer_offset(span, SPAN_HEADER_SIZE), size_class->block_count, size_class->block_size);
    ...
    static int _rpmalloc_span_finalize(heap_t* heap, size_t iclass, span_t* span, span_t** list_head)
    {
        ...
        if (span == class_span)
        {
            ...
            if (last_block) { *((void**) last_block) = free_list; }
            else { span->free_list = free_list; }
        ...
        if (span->list_size == span->used_count)
        {
            ...
            if (list_head) _rpmalloc_span_double_link_list_remove(list_head, span);
    ...
    static void _rpmalloc_global_cache_insert_spans(span_t** span, size_t span_count, size_t count)
    {
        const size_t cache_limit
            = (span_count == 1) ? GLOBAL_CACHE_MULTIPLIER * MAX_THREAD_SPAN_CACHE : GLOBAL_CACHE_MULTIPLIER * (MAX_THREAD_SPAN_LARGE_CACHE - (span_count >> 1));
...
#endif
        if ((cache->count + insert_count) > cache_limit) insert_count = cache_limit - cache->count;

        fst::memcpy(cache->span + cache->count, span, sizeof(span_t*) * insert_count);
        ...
        {
#else
        // Enable unlimited cache if huge pages, or we will leak since it is
        // unlikely that an entire huge page will be unmapped, and we're unable to
        // partially decommit a huge page
        ...
        for (size_t ispan = insert_count; ispan < count; ++ispan)
        {
            ...
            if ((current_span->flags & SPAN_FLAG_MASTER) && (atomic_load32(&current_span->remaining_spans) > (int32_t) current_span->span_count))
            {
                current_span->next = keep;
                keep = current_span;
            }
            else { _rpmalloc_span_unmap(current_span); }
        }
            ...
            while (keep)
            {
                for (; islot < cache->count; ++islot)
                {
                    ...
                    if (!(current_span->flags & SPAN_FLAG_MASTER)
                        || ((current_span->flags & SPAN_FLAG_MASTER) && (atomic_load32(&current_span->remaining_spans) <= (int32_t) current_span->span_count)))
                    {
    ...
    static size_t _rpmalloc_global_cache_extract_spans(span_t** span, size_t span_count, size_t count)
    {
        ...
        if (want > cache->count) want = cache->count;

        fst::memcpy(span + extract_count, cache->span + (cache->count - want), sizeof(span_t*) * want);
    ...
    static void _rpmalloc_heap_cache_adopt_deferred(heap_t* heap, span_t** single_span)
    {
        ...
        while (span)
        {
            ...
            {
                if (span->size_class == SIZE_CLASS_HUGE) { _rpmalloc_deallocate_huge(span); }
                else
                {
    ...
    static void _rpmalloc_heap_unmap(heap_t* heap)
    {
        ...
        {
            if (atomic_decr32(&heap->master_heap->child_count) == 0) { _rpmalloc_heap_unmap(heap->master_heap); }
        }
    }
    ...
    static void _rpmalloc_heap_global_finalize(heap_t* heap)
    {
        ...
        if (list_heap == heap) { _memory_heaps[list_idx] = heap->next_heap; }
        else
        {
    ...
    {
        span_t* span = 0;
        if (span_count == 1) { _rpmalloc_heap_cache_adopt_deferred(heap, &span); }
        else
        {
    ...
    {
        if (heap->spans_reserved >= span_count) return _rpmalloc_span_map(heap, span_count);
        return 0;
    }
    ...
    static void _rpmalloc_inc_span_statistics(heap_t* heap, size_t span_count, uint32_t class_idx)
    {
        ...
        if (current_count > (uint32_t) atomic_load32(&heap->span_use[idx].high)) atomic_store32(&heap->span_use[idx].high, (int32_t) current_count);
    ...
    static span_t* _rpmalloc_heap_extract_new_span(heap_t* heap, heap_size_class_t* heap_size_class, size_t span_count, uint32_t class_idx)
    {
        ...
        if (limit_span_count > LARGE_CLASS_COUNT) limit_span_count = LARGE_CLASS_COUNT;
        do
        {
            span = _rpmalloc_heap_thread_cache_extract(heap, span_count);
            if (EXPECTED(span != 0))
    ...
    {
        // Map in pages for a 16 heaps. If page size is greater than required size
        // for this, map a page and use first part for heaps and remaining part for
        // spans for allocations. Adds a lot of complexity, but saves a lot of
        // memory on systems where page size > 64 spans (4MiB)
        ...
        if (_memory_global_reserve_count >= heap_span_count) { span = _rpmalloc_global_get_reserved_spans(heap_span_count); }
        if (!span)
        {
        ...
        if (num_heaps < request_heap_count) num_heaps = request_heap_count;
    ...
    static heap_t* _rpmalloc_heap_allocate(int first_class)
    {
        ...
        while (!atomic_cas32_acquire(&_memory_global_lock, 1, 0))
            _rpmalloc_spin();
        if (first_class == 0) heap = _rpmalloc_heap_extract_orphan(&_memory_orphan_heaps);
#if RPMALLOC_FIRST_CLASS_HEAPS
        if (!heap) heap = _rpmalloc_heap_extract_orphan(&_memory_first_class_orphan_heaps);
#endif
        if (!heap) heap = _rpmalloc_heap_allocate_new();
        atomic_store32_release(&_memory_global_lock, 0);
        if (heap) _rpmalloc_heap_cache_adopt_deferred(heap, 0);
        return heap;
    }
    ...
    static void _rpmalloc_heap_release(void* heapptr, int first_class, int release_cache)
    {
        ...
        if (get_thread_heap_raw() == heap) set_thread_heap(0);
    ...
    static void _rpmalloc_heap_finalize(heap_t* heap)
    {
        ...
        for (size_t iclass = 0; iclass < SIZE_CLASS_COUNT; ++iclass)
        {
            if (heap->size_class[iclass].cache) _rpmalloc_span_unmap(heap->size_class[iclass].cache);
            ...
            if (heap->size_class[iclass].free_list)
            {
                ...
                {
                    if (list) _rpmalloc_span_double_link_list_remove(list, class_span);
    ...
    static void* _rpmalloc_allocate_from_heap_fallback(heap_t* heap, heap_size_class_t* heap_size_class, uint32_t class_idx)
    {
        ...
        if (EXPECTED(span != 0))
        {
            ...
            {
                // If the span did not fully initialize free list, link up another
                // page worth of blocks
                void* block_start = pointer_offset(span, SPAN_HEADER_SIZE + ((size_t) span->free_list_limit * span->block_size));
                span->free_list_limit += free_list_partial_init(&heap_size_class->free_list, &block, (void*) ((uintptr_t) block_start & ~(_memory_page_size - 1)),
            ...
            if (atomic_load_ptr(&span->free_list_deferred)) _rpmalloc_span_extract_free_list_deferred(span);

            // If span is still not fully utilized keep it in partial list and early
            // return block
            if (!_rpmalloc_span_is_fully_utilized(span)) return block;
    ...
    static void* _rpmalloc_allocate_small(heap_t* heap, size_t size)
    {
        ...
        if (EXPECTED(heap_size_class->free_list != 0)) return free_list_pop(&heap_size_class->free_list);
    ...
    static void* _rpmalloc_allocate_large(heap_t* heap, size_t size)
    {
        ...
        if (size & (_memory_span_size - 1)) ++span_count;
    ...
    static void* _rpmalloc_allocate_huge(heap_t* heap, size_t size)
    {
        ...
        if (size & (_memory_page_size - 1)) ++num_pages;
        size_t align_offset = 0;
    ...
    {
        if (alignment <= SMALL_GRANULARITY) return _rpmalloc_allocate(heap, size);
        ...
        if ((alignment <= SPAN_HEADER_SIZE) && ((size + SPAN_HEADER_SIZE) < _memory_medium_size_limit))
        {
            ...
            // two), and size aligned to span header size multiples is less than
            // size + alignment, then use natural alignment of blocks to provide
            // alignment
            ...
            if (multiple_size <= (size + alignment)) return _rpmalloc_allocate(heap, multiple_size);
        }
        ...
        if (limit_pages < (original_pages * 2)) limit_pages = original_pages * 2;
    ...
    retry:
        ...
        if ((uintptr_t) ptr & align_mask) ptr = (void*) (((uintptr_t) ptr & ~(uintptr_t) align_mask) + alignment);
    ...
    static void _rpmalloc_deallocate_direct_small_or_medium(span_t* span, void* block)
    {
        ...
        *((void**) block) = span->free_list;
        ...
        if (UNEXPECTED(span->used_count == span->list_size))
        {
            ...
            if (span->used_count)
            {
                // Make sure we have synchronized the deferred list and list size by
                // using acquire semantics and guarantee that no external thread is
                // accessing span concurrently
                void* free_list;
                do
                {
    ...
    {
        if (span->size_class != SIZE_CLASS_HUGE) _rpmalloc_stat_inc(&heap->span_use[span->span_count - 1].spans_deferred);
    ...
    static void _rpmalloc_deallocate_defer_small_or_medium(span_t* span, void* block)
    {
        ...
        *((void**) block) = free_list;
        ...
        {
            // Span was completely freed by this block. Due to the INVALID_POINTER
            // spin lock no other thread can reach this state simultaneously on this
            // span. Safe to move to owner heap deferred cache
...
// cannot touch the list or counters of another heap
        ...
        if (set_as_reserved)
        {
            ...
            if (span->flags & SPAN_FLAG_MASTER) { heap->span_reserve_master = span; }
            else
            { // SPAN_FLAG_SUBSPAN
    ...
    static void* _rpmalloc_reallocate(heap_t* heap, void* p, size_t size, size_t oldsize, unsigned int flags)
    {
        if (p)
        {
            ...
            if (EXPECTED(span->size_class < SIZE_CLASS_COUNT))
            {
                ...
                if (!oldsize) oldsize = (size_t) ((ptrdiff_t) span->block_size - pointer_diff(p, block));
                if ((size_t) span->block_size >= size)
                {
                    // Still fits in block, never mind trying to save memory, but
                    // preserve data if alignment changed
                    if ((p != block) && !(flags & RPMALLOC_NO_PRESERVE)) ::memmove(block, p, oldsize);
                    return block;
                }
            }
            else if (span->size_class == SIZE_CLASS_LARGE)
            {
                ...
                if (!oldsize) oldsize = (current_spans * _memory_span_size) - (size_t) pointer_diff(p, block) - SPAN_HEADER_SIZE;
            ...
            else
            {
                ...
                void* block = pointer_offset(span, SPAN_HEADER_SIZE);
                if (!oldsize) oldsize = (current_pages * _memory_page_size) - (size_t) pointer_diff(p, block) - SPAN_HEADER_SIZE;
        ...
        if (!!(flags & RPMALLOC_GROW_OR_FAIL)) return 0;
        ...
        if (p && block)
        {
            if (!(flags & RPMALLOC_NO_PRESERVE)) ::memcpy(block, p, oldsize < new_size ? oldsize : new_size);
    ...
    {
        if (alignment <= SMALL_GRANULARITY) return _rpmalloc_reallocate(heap, ptr, size, oldsize, flags);
        ...
        {
            if (no_alloc || (size >= (usablesize / 2))) return ptr;
        }
    ...
    static void _rpmalloc_adjust_size_class(size_t iclass)
    {
        ...
        if (iclass >= SMALL_CLASS_COUNT)
        {
            ...
            {
                --prevclass;
                // A class can be merged if number of pages and number of blocks are
                // equal
    ...
    int rpmalloc_initialize_config(const rpmalloc_config_t* config)
    {
        ...
        {
            if (_memory_config.enable_huge_pages) _memory_huge_pages = 1;
        }
        ...
        if (_memory_config.enable_huge_pages)
        {
            ...
            if (large_page_minimum) OpenProcessToken(GetCurrentProcess(), TOKEN_ADJUST_PRIVILEGES | TOKEN_QUERY, &token);
            if (token)
            {
                LUID luid;
                if (LookupPrivilegeValue(0, SE_LOCK_MEMORY_NAME, &luid))
                {
                    ...
                    {
                        if (GetLastError() == ERROR_SUCCESS) _memory_huge_pages = 1;
                    }
                }
                CloseHandle(token);
            }
            ...
            {
                if (large_page_minimum > _memory_page_size) _memory_page_size = large_page_minimum;
                if (large_page_minimum > _memory_map_granularity) _memory_map_granularity = large_page_minimum;
            }
        }
#endif
...
#endif
        if (_memory_page_size < min_span_size) _memory_page_size = min_span_size;
        if (_memory_page_size > max_page_size) _memory_page_size = max_page_size;
        ...
        else
        {
            ...
            if (span_size > (256 * 1024)) span_size = (256 * 1024);
        ...
        if ((_memory_span_size * _memory_span_map_count) < _memory_page_size) _memory_span_map_count = (_memory_page_size / _memory_span_size);
...
#if ((defined(__APPLE__) || defined(__HAIKU__)) && ENABLE_PRELOAD) || defined(__TINYC__)
        if (pthread_key_create(&_memory_thread_heap, _rpmalloc_heap_release_raw_fc)) return -1;
#endif
#if defined(_WIN32) && (!defined(BUILD_DYNAMIC_LINK) || !BUILD_DYNAMIC_LINK)
        ...
        if (_memory_medium_size_limit > MEDIUM_SIZE_LIMIT) _memory_medium_size_limit = MEDIUM_SIZE_LIMIT;
    ...
    {
        heap_t* heap = get_thread_heap_raw();
        if (heap) _rpmalloc_heap_release_raw(heap, release_caches);
        set_thread_heap(0);
#if defined(_WIN32) && (!defined(BUILD_DYNAMIC_LINK) || !BUILD_DYNAMIC_LINK)
    ...
    void rpmalloc_thread_statistics(rpmalloc_thread_statistics_t* stats)
    {
        ...
        for (size_t iclass = 0; iclass < SIZE_CLASS_COUNT; ++iclass)
        {
            ...
            while (span)
            {
                ...
                if (span->free_list_limit < block_count) block_count = span->free_list_limit;
        ...
        while (deferred)
        {
            if (deferred->size_class != SIZE_CLASS_HUGE) stats->spancache += (size_t) deferred->span_count * _memory_span_size;
    ...
    {
        fprintf((FILE*) file, "Heap %d stats:\n", heap->id);
        fprintf((FILE*) file, "Class   CurAlloc  PeakAlloc   TotAlloc    TotFree  BlkSize "
                              "BlkCount SpansCur SpansPeak  PeakAllocMiB  ToCacheMiB "
                              "FromCacheMiB FromReserveMiB MmapCalls\n");
        for (size_t iclass = 0; iclass < SIZE_CLASS_COUNT; ++iclass)
        {
            if (!atomic_load32(&heap->size_class_use[iclass].alloc_total)) continue;
            fprintf((FILE*) file,
                "%3u:  %10u %10u %10u %10u %8u %8u %8d %9d %13zu %11zu %12zu "
                "%14zu %9u\n",
                (uint32_t) iclass, atomic_load32(&heap->size_class_use[iclass].alloc_current), heap->size_class_use[iclass].alloc_peak,
                atomic_load32(&heap->size_class_use[iclass].alloc_total), atomic_load32(&heap->size_class_use[iclass].free_total), _memory_size_class[iclass].block_size,
                _memory_size_class[iclass].block_count, atomic_load32(&heap->size_class_use[iclass].spans_current), heap->size_class_use[iclass].spans_peak,
        ...
        fprintf((FILE*) file, "Spans  Current     Peak Deferred  PeakMiB  Cached  ToCacheMiB "
                              "FromCacheMiB ToReserveMiB FromReserveMiB ToGlobalMiB "
                              "FromGlobalMiB  MmapCalls\n");
        for (size_t iclass = 0; iclass < LARGE_CLASS_COUNT; ++iclass)
        {
            if (!atomic_load32(&heap->span_use[iclass].high) && !atomic_load32(&heap->span_use[iclass].spans_map_calls)) continue;
            fprintf((FILE*) file,
                "%4u: %8d %8u %8u %8zu %7u %11zu %12zu %12zu %14zu %11zu %13zu "
                "%10u\n",
                (uint32_t) (iclass + 1), atomic_load32(&heap->span_use[iclass].current), atomic_load32(&heap->span_use[iclass].high),
        ...
        fprintf((FILE*) file, "Full spans: %zu\n", heap->full_span_count);
        fprintf((FILE*) file, "ThreadToGlobalMiB GlobalToThreadMiB\n");
        fprintf((FILE*) file, "%17zu %17zu\n", (size_t) atomic_load64(&heap->thread_to_global) / (size_t) (1024 * 1024),
    ...
    void rpmalloc_dump_statistics(void* file)
    {
        ...
        for (size_t list_idx = 0; list_idx < HEAP_ARRAY_SIZE; ++list_idx)
        {
            ...
            while (heap)
            {
                ...
                if (need_dump) _memory_heap_dump_statistics(heap, file);
                heap = heap->next_heap;
            }
        }
        fprintf((FILE*) file, "Global stats:\n");
        ...
        fprintf((FILE*) file, "HugeCurrentMiB HugePeakMiB\n");
        fprintf((FILE*) file, "%14zu %11zu\n", huge_current / (size_t) (1024 * 1024), huge_peak / (size_t) (1024 * 1024));

        fprintf((FILE*) file, "GlobalCacheMiB\n");
        for (size_t iclass = 0; iclass < LARGE_CLASS_COUNT; ++iclass)
        {
            ...
            if (global_cache || global_overflow_cache || cache->insert_count || cache->extract_count)
                fprintf((FILE*) file,
                    "%4zu: %8zuMiB (%8zuMiB overflow) %14zu insert %14zu "
                    "extract\n",
        ...
        fprintf((FILE*) file, "MappedMiB MappedOSMiB MappedPeakMiB MappedTotalMiB "
                              "UnmappedTotalMiB\n");
        fprintf((FILE*) file, "%9zu %11zu %13zu %14zu %16zu\n", mapped / (size_t) (1024 * 1024), mapped_os / (size_t) (1024 * 1024), mapped_peak / (size_t) (1024 * 1024),
	...
	fprintf((FILE*)file, "Allocation count: %lli\n", allocated);
	fprintf((FILE*)file, "Deallocation count: %lli\n", deallocated);
	fprintf((FILE*)file, "Current allocations: %lli\n", (allocated - deallocated));
	fprintf((FILE*)file, "Master spans: %d\n", atomic_load32(&_master_spans));
	fprintf((FILE*)file, "Dangling master spans: %d\n", atomic_load32(&_unmapped_master_spans));
#endif
#endif
        (void) sizeof(file);
    }
    ...
    {
        if (heap) _rpmalloc_heap_release(heap, 1, 1);
    }
    ...
    extern inline RPMALLOC_ALLOCATOR void* rpmalloc_heap_aligned_alloc(rpmalloc_heap_t* heap, size_t alignment, size_t size)
    {
#if ENABLE_VALIDATE_ARGS
        if (size >= MAX_ALLOC_SIZE)
        {
    ...
    extern inline RPMALLOC_ALLOCATOR void* rpmalloc_heap_realloc(rpmalloc_heap_t* heap, void* ptr, size_t size, unsigned int flags)
    {
#if ENABLE_VALIDATE_ARGS
        if (size >= MAX_ALLOC_SIZE)
        {
    ...
    extern inline RPMALLOC_ALLOCATOR void* rpmalloc_heap_aligned_realloc(rpmalloc_heap_t* heap, void* ptr, size_t alignment, size_t size, unsigned int flags)
    {
#if ENABLE_VALIDATE_ARGS
        if ((size + alignment < size) || (alignment > _memory_page_size))
        {
    ...
    extern inline void rpmalloc_heap_thread_set_current(rpmalloc_heap_t* heap)
    {
        ...
        {
            set_thread_heap(heap);
            if (prev_heap) rpmalloc_heap_release(prev_heap);
        }
    }
