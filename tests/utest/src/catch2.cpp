...
FST_PRAGMA_PUSH()
FST_PRAGMA_DISABLE_WARNING_MSVC(4866) // compiler may not enforce left-to-right evaluation order for call to operator_name
FST_PRAGMA_DISABLE_WARNING_MSVC(4868) // compiler may not enforce left-to-right evaluation order in braced initializer list
...
void ConsoleReporter::sectionStarting(SectionInfo const& _sectionInfo) {
    ...
    //m_stream << "[  SECTION  ] " << _sectionInfo.name << std::endl;

}
void ConsoleReporter::testCaseStarting(TestCaseInfo const& _testInfo)
{
    m_stream << "[   TEST   ] " << _testInfo.name << std::endl;

    //m_stream << m_colour->guardColour(Colour::BrightGreen) << "[ OK ] " << m_colour->guardColour(Colour::None) << _testInfo.name <<  std::endl;
    //m_stream << _testInfo.name << "\n" << std::flush;
...
void ConsoleReporter::testCaseEnded(TestCaseStats const& _testCaseStats) {
    ...
    StreamingReporterBase::testCaseEnded(_testCaseStats);
    m_headerPrinted = false;
    //m_stream << m_colour->guardColour(Colour::BrightGreen) << "[       OK ] > " << m_colour->guardColour(Colour::None) << _testCaseStats.testInfo->name << " " << _testCaseStats.totals.assertions.total() << std::endl << std::endl;
