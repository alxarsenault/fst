...
namespace Catch {
    ...
    template<class Fct>
class TemplateTestInvoler :   public ITestInvoker
    {
    public:
    TemplateTestInvoler(Fct&& fct): _fct(std::forward<Fct>(fct))
        {
        }
    ...
    Fct _fct;
        ...
        virtual ~TemplateTestInvoler() = default;
    };

    template<class Fct>
    inline Detail::unique_ptr<ITestInvoker> make_test_invoker(Fct&& fct)
    {
        return Detail::unique_ptr<ITestInvoker>(new TemplateTestInvoler<Fct>(std::forward<Fct>(fct)));
    }
