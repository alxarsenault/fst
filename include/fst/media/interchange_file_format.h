...
union iff_four_cc
{
    ...
    inline constexpr bool operator==(iff_four_cc h) const
    {
        return h.integer == integer;
    }

    inline constexpr bool operator==(const char* buffer) const
    {
        return iff_four_cc(buffer).integer == integer;
    }
...
struct wav_format_header
{
    ...
    static inline wav_format_header create(int16_t channel_size, uint32_t sampling_rate, __fst::audio_format_type ft)
    {
        ...
        return wav_format_header{
            __fst::is_floating_point_audio_format(ft) ? wav_audio_format::ieee : wav_audio_format::pcm,
            channel_size,
            sampling_rate,
            (uint32_t)bytes_per_block * sampling_rate,
            bytes_per_block,
            bit_depth
        };
    }
